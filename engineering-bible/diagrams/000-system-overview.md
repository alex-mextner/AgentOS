# AgentOS System Overview Spec

## Status

Active bootstrap spec. This document is the first authoritative product and
architecture scope for AgentOS. More specific subsystem specs must refine this
document rather than silently diverge from it.

## Observable System Change

AgentOS exists to produce a working mobile OS prototype whose MVP boots inside a
...
AgentOS follows a Fuchsia-inspired split:
...
1. `005-source-session-assignment.md`: recovered original AgentOS assignment,
...
- A source trace tying the AgentOS vision to the recovered source session.
